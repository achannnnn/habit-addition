...
public class LocalNotificationsPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    private func rejectCall(_ call: CAPPluginCall, _ message: String, _ code: String? = nil, _ error: Error? = nil) {
        // CAPPluginCall.reject may be unavailable depending on Capacitor SwiftPM binary API.
        if let error {
    ...
    private func getJSObjectArray(_ call: CAPPluginCall, key: String) -> [JSObject]? {
        let values = call.getArray(key, [])
        if values.isEmpty {
            return nil
        }

        return values.compactMap { $0 as? JSObject }
    }
    ...
    @objc func schedule(_ call: CAPPluginCall) {
        guard let notifications = getJSObjectArray(call, key: "notifications") else {
        ...
        for notification in notifications {
            ...
            do {
                ...
                CAPLog.print(error.localizedDescription)
                rejectCall(call, "Unable to make notification", nil, error)
                return
            }
            ...
            do {
                ...
                rejectCall(call, "Unable to create notification, trigger failed", nil, error)
                return
            }
    ...
    @objc func cancel(_ call: CAPPluginCall) {
        guard let notifications = getJSObjectArray(call, key: "notifications"), notifications.count > 0 else {
    ...
    @objc func registerActionTypes(_ call: CAPPluginCall) {
        guard let types = getJSObjectArray(call, key: "types") else {
            return
        }
    ...
    func makeAttachmentUrl(_ path: String) -> URL? {
        ...
        // Fallback for SwiftPM builds where bridge.localURL(fromWebURL:) may not be exposed.
        return webURL
    }
